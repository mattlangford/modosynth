...
public:
    Stream()
        : end_time_(0), raw_samples_(100, true), output_(std::make_unique<ThreadSafeBuffer>(Samples::kSampleRate)) {}
    ...
    void flush_samples(const std::chrono::nanoseconds end_time) {
        for (size_t i = 0; i <= index_of_timestamp(end_time); ++i) {
    ...
    size_t index_of_timestamp(const std::chrono::nanoseconds& timestamp) {
        ...
        if (timestamp < start_time) return 0;
...
private:
    ...
    std::chrono::nanoseconds end_time_;
